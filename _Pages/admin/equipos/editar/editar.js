...
import {obtenerEquipo, actualizarEquipo} from './servidor'
import estilos from './editar.module.css'

export default function EditarEquipoAdmin() {
    ...
    const cargarDatos = async () => {
        try {
            const resultado = await obtenerEquipo(equipoId)
            if (resultado.success) {
                ...
                setTotalActivos(resultado.totalActivos)
                ...
                setCategoriaId(equipo.categoria_id ? equipo.categoria_id.toString() : '')
                setMarcaId(equipo.marca_id ? equipo.marca_id.toString() : '')
                setUnidadMedidaId(equipo.unidad_medida_id ? equipo.unidad_medida_id.toString() : '')
                setPrecioCompra(equipo.precio_compra || '')
                setPrecioVenta(equipo.precio_venta || '')
                setPrecioOferta(equipo.precio_oferta || '')
                setPrecioMayorista(equipo.precio_mayorista || '')
                setCantidadMayorista(equipo.cantidad_mayorista || '6')
                setAplicaItbis(equipo.aplica_itbis !== false)
                setActivo(equipo.activo !== false)
                setTipoActivo(equipo.tipo_activo || 'vehiculo')
                setPermiteFinanciamiento(equipo.permite_financiamiento !== false)
                setMesesMaxFinanciamiento(equipo.meses_max_financiamiento ? equipo.meses_max_financiamiento.toString() : '24')
                setMesesGarantia(equipo.meses_garantia ? equipo.meses_garantia.toString() : '12')
                setTasaDepreciacion(equipo.tasa_depreciacion ? equipo.tasa_depreciacion.toString() : '')
                
                // Control de inventario
                setStockActual(resultado.stockDisponible || 0)
                setStockMinimo(equipo.stock_minimo ? equipo.stock_minimo.toString() : '5')
                setStockMaximo(equipo.stock_maximo ? equipo.stock_maximo.toString() : '100')
                setUbicacionBodega(equipo.ubicacion_bodega || '')
                
                if (equipo.imagen_url) {
                    if (equipo.imagen_url.startsWith('data:image')) {
                        setTipoImagen('local')
                        setVistaPrevia(equipo.imagen_url)
                    } else {
                        setTipoImagen('url')
                        setImagenUrl(equipo.imagen_url)
                        setVistaPrevia(equipo.imagen_url)
                    }
                }
            } else {
    ...
    const manejarSubmit = async (e) => {
        ...
        try {
            const datosEquipo = {
                ...
                imagen_base64: tipoImagen === 'local' && imagenArchivo ? vistaPrevia : (tipoImagen === 'local' && vistaPrevia && !imagenUrl ? vistaPrevia : null),
            ...
            const resultado = await actualizarEquipo(equipoId, datosEquipo)

            if (resultado.success) {
                alert(resultado.mensaje)
                router.push('/admin/equipos')
            } else {
                alert(resultado.mensaje || 'Error al actualizar equipo')
            }
        } catch (error) {
    ...
    return (
        <div className={`${estilos.contenedor} ${estilos[tema]}`}>
            <div className={estilos.header}>
                <div>
                    <h1 className={estilos.titulo}>Editar Equipo</h1>
                    <p className={estilos.subtitulo}>Actualiza la información del equipo</p>
                </div>
                <button
            ...
            {totalActivos > 0 && (
                <div className={estilos.infoActivos}>
                    <h4>Información de Activos</h4>
                    <p>Este equipo tiene {totalActivos} activo(s) asociado(s). Los cambios afectarán solo al producto base, no a los activos individuales.</p>
                </div>
            )}
                    ...
                    <div className={estilos.columnaIzquierda}>
                        ...
                        <div className={`${estilos.seccion} ${estilos[tema]}`}>
                            ...
                            <div className={estilos.alertaInfo}>
                                <ion-icon name="information-circle-outline"></ion-icon>
                                <p>
                                    Stock actual: <strong>{stockActual} unidades disponibles</strong>. 
                                    Los valores de stock mínimo y máximo sirven para generar alertas automáticas.
                                </p>
                            </div>
                            ...
                            <div className={estilos.gridTresColumnas}>
                                <div className={estilos.grupoInput}>
                                    ...
                                    <small style={{color: '#64748b', fontSize: '12px'}}>
                                        Alerta cuando haya menos de esta cantidad
                                    </small>
                                </div>
                                ...
                                <div className={estilos.grupoInput}>
                                    ...
                                    />
                                    <small style={{color: '#64748b', fontSize: '12px'}}>
                                        Alerta cuando supere esta cantidad
                                    </small>
                                </div>
                                ...
                                <div className={estilos.grupoInput}>
                                    ...
                                    />
                                    <small style={{color: '#64748b', fontSize: '12px'}}>
                                        Ubicación predeterminada para activos
                                    </small>
                                </div>
                            </div>
                        </div>
