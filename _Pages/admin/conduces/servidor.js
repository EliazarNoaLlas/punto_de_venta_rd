    ...
    try {
        ...
        if (tipo === 'venta') {
            [res] = await connection.execute(
                `SELECT v.id, v.numero_interno as numero, c.nombre as cliente_nombre, v.cliente_id
                 FROM ventas v
                 LEFT JOIN clientes c ON v.cliente_id = c.id
                 WHERE v.numero_interno = ? AND v.empresa_id = ?`,
                [numero, empresaId]
            )
        } else {
            [res] = await connection.execute(
                `SELECT c.id, c.numero_cotizacion as numero, cl.nombre as cliente_nombre, c.cliente_id
                 FROM cotizaciones c
                 LEFT JOIN clientes cl ON c.cliente_id = cl.id
                 WHERE c.numero_cotizacion = ? AND c.empresa_id = ?`,
                [numero, empresaId]
            )
        }
