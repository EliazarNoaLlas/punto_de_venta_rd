...
export default function CheckoutPublico() {
    ...
    const cargarConfig = async () => {
        try {
            const respuesta = await fetch(`/api/catalogo-superadmin/${slug}/config`)
            const resultado = await respuesta.json()
            if (resultado.success) {
                setConfig(resultado.config)
            } else {
                console.error('Error al cargar config:', resultado.mensaje)
                router.push(`/catalogo-superadmin/${slug}`)
            }
        } catch (error) {
    ...
    const cargarCarritoDesdeStorage = () => {
        const carritoGuardado = localStorage.getItem(`carrito_catalogo_superadmin_${slug}`)
        if (carritoGuardado) {
            try {
    ...
    const handleSubmit = async (e) => {
        ...
        setProcesando(true)
        try {
            const respuesta = await fetch(`/api/catalogo-superadmin/${slug}/pedido`, {
                method: 'POST',
                headers: {
            ...
            if (resultado.success) {
                ...
                localStorage.removeItem(`carrito_catalogo_superadmin_${slug}`)
                
                // Si hay URL de WhatsApp, abrirla automáticamente
            ...
            <div className={estilos.contenedor}>
                <div className={estilos.pedidoConfirmado}>
                    ...
                    <div className={estilos.accionesConfirmacion}>
                        ...
                        <Link href={`/catalogo-superadmin/${slug}`} className={estilos.btnVolver}>
                            Volver al Catálogo
                        </Link>
                    </div>
                </div>
        ...
        <div className={estilos.contenedor}>
            <header className={estilos.header}>
                <div className={estilos.headerContenedor}>
                    <Link href={`/catalogo-superadmin/${slug}`} className={estilos.btnVolverHeader}>
